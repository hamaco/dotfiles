// ==UserScript==
// @name        Minibuffer opera
// @namespace   http://white.s151.xrea.com/
// @description Minibuffer on Opera
// @checkurl    http://ss-o.net/userjs/minibuffer.opera.js
// @include     *
// ==/UserScript==
(function(){
function _minibuffer() {
var VERSION = "2008.03.21";
...
//  32-40 space pageup pagedown end home left up right down
  ...
  findByEvent: function(e, ch){return $A.find(values(this.hash),function(kf){return kf[0].equal(e, ch) && kf})},
...
// enable/disable temporary
  ...
  listener: function(aEvent){
	  ...
	  */
		var ch = (function(self){
			var key = String.fromCharCode(aEvent.which);
			if (self.otherStr[aEvent.keyCode] && (aEvent.which == aEvent.keyCode)) return key;
			return self.keyCodeStr[aEvent.keyCode] || self.whichStr[aEvent.which] || key;
		})(this);
...
//	  log(aEvent.keyCode, aEvent.which, ch,kf, aEvent.shiftKey);
		//_log(aEvent.keyCode, aEvent.which, ch,kf, aEvent.shiftKey,preventDefault,ch.length);
	  if(kf){
		  var fn = kf[1];
...
Minibuffer.prototype = {
  ...
  initialize: function(){
	  ...
	  //this.shortcutkey.addEventListener(this.html.input, 'keypress', true);
		if (window.opera) {
			this.shortcutkey.addEventListener(this.html.input, 'keypress', false);
		} else {
			this.shortcutkey.addEventListener(this.html.input, 'keydown', true);
		}
	  var self = this;
	  var chars = "!\"#$%&'()=~|-^_[]{}:;+*@`?\\".split('');
...
// html
  initHTML: function(){
	  ...
	  this.html.input = $N('textarea',{id:INPUT_ID,rows:'1'});
  ...
  updateComplationList: function(scroll){
	  ...
	  var test = function(str, l){
		  ...
		  return forFilter(l,function(el){return el.match(regexp)});
	  }
	  // all candidates
	  if(input_str == '') new_lst = old_lst;
	  // prefix match
	  if(!new_lst.length) new_lst = test('^' + escapeRegexp(input_str.replace(/^\^/, '')), old_lst);
	  // substring match
	  if(!new_lst.length) new_lst = test(escapeRegexp(input_str), old_lst);
  ...
  scrollPageNext: function(stop){ // stop at the bottom
	  ...
	  if(this.MAX_CANDIDATES < lst.length){
		  ...
		  c.innerHTML = '';
		  forEach(next_candidates,function(e){
  ...
  scrollPagePrev: function(stop){ // stop at the top
	  ...
	  if(this.MAX_CANDIDATES < lst.length){
		  ...
		  var pos = $A.position(lst,txt) - this.MAX_CANDIDATES;
		  if(pos < 0){
			  if(stop) return false;
  ...
  selectCandidate: function(newNode, oldNode){ // highlight node
	  ...
	  if(newNode){
		  i.value = (i.value.match(new RegExp('.*'+escapeRegexp(this.separator)+'\\s*')) || '') + newNode.innerHTML;
...
//	minibuffer.addEventListener(obj);
  ...
  removeEventListener: function(obj){
	  this.listeners = $A.remove(this.listeners,obj);
  },
...
// ShortcutKey
  ...
  bindComplete: function(){
	  ...
	  var setString = function(str){
		  ...
		  // "a | b | c" => "a | b | "
		  var pstr = i.value.slice(0,b).match(new RegExp('.*'+escapeRegexp(self.separator)+'\\s*'));
  ...
  bindDecision: function(){
	  ...
	  if(this.hasOwnProperty('history')){ // ensure to set setter/getter
		  ...
		  history = $A.remove(history,string);// to avoid duplicated
...
// history
  bindSearchHistoryBackward: function(){
	  ...
	  var count = $A.position(history,function(e, n){
  ...
  bindSearchHistoryForward: function(){
	  ...
	  var count = $A.position(history.slice(0,this.history_search_count).reverse(),function(e, i){return e.match(self.history_search_regexp)});
...
var FlashMessage = new function(){
	var initial = function(){
		...
		var opacity = 0.9;
		flash = $N('div',{id:'FLASH_MESSAGE'});
		hide(flash);
		document.body.appendChild(flash);
	};
	var canceler, flash, init = true;
...
Status.prototype = {
  initialize: function(){
	  ...
	  var name  = arguments[0] ,status = arguments[1] ,time_limit = arguments[2];
	  ...
	  if(typeof status != "string"){
		  ...
		  if(typeof time_limit == 'number'){
			  FunctionLater(this.fadeout,time_limit).call(this, hash[name]);
			  FunctionLater(del,time_limit)();
		  }
	  }
  },
  ...
  fadeout: function(node){
	  ...
	  var base = 1000;
	  for(var i=0; i<max; i++){
		  FunctionLater(setOpacity,i/max*base)(node, 1-i/max);
	  }
	  FunctionLater(this.remove,base*1.2)(node);
  },
...
//	  var argument = hash['argument'];
  ...
  execute: function(commandline, stdin){
	  ...
	  var ret = forEach(commands,function(command){
...
function $X (exp, context) {
	var type, namespace={};
	if (window.getMatchedCSSRules) {//for Safari from jAutoPagerize ( http://coderepos.org/share/log/lang/javascript/userscripts/jautopagerize.user.js? )
...
var getSelectionText = function(){
	return $A.remove(String(window.getSelection()).split(/\r?\n|\r/),"");
}
...
//return this.replace(/^\?/, '.?').replace(/^\*/, '.*').replace(/\|/g,'\\|').replace(/\+/g,'\\+').replace(/\([^)]*$/, '').replace(/\[[^\]]*$/, '');
//}
function escapeRegexp(str){
  return  str.replace(/^\?/, '.?').replace(/^\*/, '.*').replace(/\|/g,'\\|').replace(/\+/g,'\\+').replace(/\([^)]*$/, '').replace(/\[[^\]]*$/, '');
}
/*
...
Array.prototype.remove = function(obj){
	var test = (typeof(obj) == 'function') ? obj : function(a){return a == obj};
	return forFilter(this,function(e){return !test(e)})
}
...
*/
var $A = {};
$A.position = function(arr,obj){
	var test = (typeof(obj) == 'function') ? obj : function(a){return a == obj};
	for(var i=0;i<arr.length; i++) if(test(arr[i], i)) return i;
	return false;
}
$A.last = function(){
	return this[this.length-1];
}
$A.find = function(arr,obj){
	var i = $A.position(arr,obj);
	return typeof(i) == 'number' ? arr[i] : false;
}
$A.remove = function(arr,obj){
	var test = (typeof(obj) == 'function') ? obj : function(a){return a == obj};
	return forFilter(arr,function(e){return !test(e)})
}
$A.reduce = function(arr,fn ,initial){
	var len = arr.length;
	if(typeof fn != "function" || (len == 0 && arguments.length == 2)) throw new TypeError();
	var i = 0;
	...
	for (;i<len;i++) if(i in arr) rv=fn.call(null, rv, arr[i], i, arr);
	return rv;
}
/*
...
*/
function FunctionLater(func,ms){
	return function(){
		var args = arguments;
		var thisObject = this;
		var res = {
...
//FunctionLater(function(){console.log(this,arguments);},100,{a:1})(1,2,3);

// usage:
//   var lst = (function(e){console.log(e)}).eachLater(500)([1],[2],[3],[4],[5],[6],[7]);
//   (function(){forEach(lst,function(e){e.complete || e.cancel()})}).later(2000)();
...
Function.prototype.eachLater = function(ms){
	...
	return function(){
		var tmp=0, lst=[];
		for(var i=0;i<arguments.length; i++) lst[lst.length] = FunctionLater(self,tmp+=ms).apply(this,arguments[i]);
		return lst;
	}
}
*/
function FunctionEachLater(func,ms){
	return function(){
		var tmp=0, lst=[];
		for(var i=0;i<arguments.length; i++) lst[lst.length] = FunctionLater(func,tmp+=ms).apply(this,arguments[i]);
		return lst;
	}
}
...
if(document.body){
	...
	window.Minibuffer.addCommand({
		name: 'innerHTML',
		command: function(stdin){return forMap(stdin,function(a){return a.innerHTML})}
	});
	...
	window.Minibuffer.addCommand({
		...
		command: function(stdin){
			...
			return forMap(urls,function(url){
	...
	window.Minibuffer.addCommand({
		...
		command: function (stdin) {
			...
			return forFilter(stdin,function(obj) {
	...
	window.Minibuffer.addCommand({
		...
		command: function(stdin){
			...
			forEach(stdin,function(url){
				openInTab(url,target);
			});
			return stdin;
		}
	});
	...
	window.Minibuffer.addCommand({
		name: 'web-archive',
		command: function(stdin){
			return forMap(stdin,function(url){return 'http://web.archive.org/web/*/' + url})
		}
	});
	...
	window.Minibuffer.addCommand({
		name: 'google-cache',
		command: function(stdin){
			return forMap(stdin,function(url){return 'http://www.google.com/search?q=cache:' + url})
		}
	});
	...
	window.Minibuffer.addCommand({
		name: 'web-gyotaku',
		command: function(stdin){
			return forMap(stdin,function(url){return 'http://megalodon.jp/?url=' + url})
		}
	});
	...
	window.Minibuffer.addCommand({
		...
		command: function(stdin){
			window.scrollTo(0, window.scrollMaxY || document.documentElement.scrollHeight - document.body.clientHeight);
			return stdin;
		}
	});
...
//		name: 'tako3',
//		command: function(stdin){
//			return forMap(stdin,function(url){return "http://tako3.com/" + url})
//		}
//	});
...
function GM_addStyle(css) {
	...
	var root = head.length ? head[0] : document.body;
	if (root) {
...
function forEach(arr,f,o){
	for (var i=0,l=arr.length;i<l;++i) f.call(o,arr[i],i,arr);
}
function forFilter(arr,f,o){
	for (var i=0,l=arr.length,r=[];i<l;++i) if(f.call(o,arr[i],i,arr)) r.push(arr[i]);
	return r;
}
function forMap(arr,f,o){
	for (var i=0,l=arr.length,r=[];i<l;++i) r.push(f.call(o,arr[i],i,arr));
	return r;
}
function _log(){
	var r = [];
	...
	window.Minibuffer.log.push(r.join(',') + '\n');
}
function openInTab(url,target){
	...
	if (window.opera) {
		try {
			...
			$N('a', {href:url, target:target} ).dispatchEvent(evt);
		} catch (e) {
			return false;
		}
		return true;
...
if(window.opera) document.addEventListener('DOMContentLoaded',_minibuffer,false);
else _minibuffer();
