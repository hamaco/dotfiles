...
// @description    loading next page and inserting into current page. oAutoPagerize supports for Opera 9.5+, GreaseKit(Safari/WebKit) and Greasemetal(Google Chrome).
// @version        1.2.3
// @include        http://*
// @include        https://*
...
(function(contentWindow){
	var Set = contentWindow.AutoPagerizeSettings || {
		TOP_SITEINFO:[]
		,BOTTOM_SITEINFO:[]
	};
	var HTML_NAMESPACE = 'http://www.w3.org/1999/xhtml';
	Set.DISABLE_SITE = Set.DISABLE_SITE || [];
	if (contentWindow.AutoPagerizeWedataSiteinfo) {
		var SITEINFO = contentWindow.AutoPagerizeWedataSiteinfo;
	} else {
		var fn = arguments.callee;
		insertSITEINFO(function(siteinfo){
			contentWindow.AutoPagerizeWedataSiteinfo = siteinfo;
			fn(contentWindow);
		});
		return;
	}
	...
	contentWindow.AutoPagerizeSettings = null;
	contentWindow.AutoPagerizeWedataSiteinfo = null;
	...
	if (location.host == 'eow.alc.co.jp') {
		...
		miscellaneous.push(function(){
			alc(document);
			setTimeout(function(){if (contentWindow.AutoPagerize) AutoPagerize.addDocumentFilter(alc);},500);
		});
	}
	if (HISTORY_MODE_FAST && window.opera) {
		opera.addEventListener('AfterEvent.GM_AutoPagerizeNextPageLoaded', function(){
			if (contentWindow.AutoPagerize)
	...
	var autopager = function() {
		miscellaneous.forEach(function(f){f();});
		...
		AutoPager.prototype = {
			...
			,request:function() {
				...
				if (window.opera && ( !window.postMessage || 'utf-8' != document.characterSet ) ) {
					this.frameRequest();
				} else {
					this.httpRequest();
				}
		...
		// utility functions.
		contentWindow.AutoPagerize.getElementsByXPath = getElementsByXPath;
		contentWindow.AutoPagerize.getFirstElementByXPath = getFirstElementByXPath;
	...
	function insertSITEINFO(callback,thisObject){
		...
		contentWindow.AutoPagerizeCallbackSiteinfo = function(res){
			callback.call(thisObject,res);
			contentWindow.AutoPagerizeCallbackSiteinfo = null;
		};
		sc.src = src;
